...
def download_m4a(video_id: str, dst_dir: pathlib.Path, base_name: str) -> pathlib.Path:
	"""
	...
	rc = _run([ffmpeg_path(), "-y", "-i", str(src), "-vn", "-sn", "-c:a", "copy", str(dst)])
	...
	rc = _run([ffmpeg_path(), "-y", "-i", str(src), "-vn", "-sn", "-c:a", "aac", "-b:a", "192k", str(dst)])
	try: src.unlink()
	except Exception: pass
...
def download_mp3(video_id: str, dst_dir: pathlib.Path, base_name: str, cbr_320: bool = False) -> pathlib.Path:
	dst_dir.mkdir(parents=True, exist_ok=True)
	safe_base = _safe(base_name)
